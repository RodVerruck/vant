...
useEffect(() => {
    ...
    console.log("[URL Params] Verificando retorno do Stripe:", { payment, sessionId: sessionId?.slice(0, 20) });

    if (payment === "success" && sessionId) {
        console.log("[URL Params] Pagamento confirmado, verificando sessão...");
        setStage("checkout");
        setCheckoutError("");
        ...
        (async () => {
            try {
                ...
                console.log("[URL Params] Resposta verify-checkout-session:", payload);
                ...
                // Em ambiente de teste, aceitar mesmo se paid for false
                // O backend vai criar a assinatura manualmente
                const isPaid = payload.paid === true || payload.payment_status === "unpaid" || payload.status === "open";
                console.log("[URL Params] Pagamento verificado:", { isPaid, paid: payload.paid, payment_status: payload.payment_status, status: payload.status });
                ...
                if (isPaid) {
                    ...
                    // Salvar sessão e ativar
                    console.log("[URL Params] Setando stripeSessionId:", sessionId.slice(0, 20));
                    setStripeSessionId(sessionId);

                    // Pequeno delay para garantir que stripeSessionId foi setado
                    await new Promise(resolve => setTimeout(resolve, 100));

                    if (!authUserId) {
                        // Sem usuário: salvar e pedir login
                        console.log("[URL Params] Sem usuário, salvando sessão no localStorage");
                        window.localStorage.setItem("vant_pending_stripe_session_id", sessionId);
                        setNeedsActivation(true);
                        setCheckoutError("Pagamento confirmado. Faça login para finalizar a ativação do seu plano.");
                        setStage("checkout");
                    } else {
                        // Com usuário: ativar imediatamente
                        console.log("[URL Params] Com usuário, ativando...");
                        setNeedsActivation(true);
                    }

                    // Limpar URL apenas após sucesso
                ...
                console.error("[URL Params] Erro ao verificar pagamento:", e);
...
useEffect(() => {
    ...
    console.log("[useEffect needsActivation] Estado:", { needsActivation, authUserId, stripeSessionId: stripeSessionId?.slice(0, 20), isActivating });
    if (!needsActivation || !authUserId || !stripeSessionId || isActivating) {
        console.log("[useEffect needsActivation] Retornando porque:", {
            needsActivation_false: !needsActivation,
            authUserId_null: !authUserId,
            stripeSessionId_null: !stripeSessionId,
            isActivating_true: isActivating
        });
        return;
    }
