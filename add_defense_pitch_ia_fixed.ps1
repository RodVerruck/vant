# Script para adicionar geração de Pitch de Defesa por IA com fallback (corrigido)

$filePath = "frontend/src/components/PaidStage.tsx"
$content = Get-Content $filePath -Raw

# Add state for defense pitch
$statePattern = 'const \[copiedField, setCopiedField\] = useState<string \| null>\(null\);'
$stateReplacement = @'
...
$functionPattern = 'const technicalQuality = calculateTechnicalQuality\(\);'
$functionReplacement = @'
...
# Update UI to add loading indicator
$uiPattern = '<h3 className="vant-h3">💡 Sua Estratégia de Defesa na Entrevista</h3>\s*<p className="vant-text-sm vant-text-slate-400">Argumento pronto para explicar gaps de senioridade</p>'
$uiReplacement = @'
...
$content = $content -replace '"Como justificar o gap de senioridade: .Embora meu cargo anterior fosse focado em Suporte, liderei a retenção de clientes críticos durante crises. Isso desenvolveu minha resiliência e visão de Customer Success, que são fundamentais para este desafio de Growth.\.'"', '{defensePitchLoading ? (<span style={{ color: "#94a3b8", fontStyle: "italic" }}>Gerando argumento personalizado...</span>) : (`"Como justificar o gap de senioridade: '${defensePitch}'"`)}'

# Update copy button
$copyPattern = 'onClick=\(\(\) => navigator\.clipboard\.writeText\("Como justificar o gap de senioridade: .Embora meu cargo anterior fosse focado em Suporte, liderei a retenção de clientes críticos durante crises. Isso desenvolveu minha resiliência e visão de Customer Success, que são fundamentais para este desafio de Growth.\."\)\)'
$copyReplacement = 'onClick={() => navigator.clipboard.writeText(`Como justificar o gap de senioridade: "${defensePitch}"`)}'
$content = $content -replace $copyPattern, $copyReplacement
